...
def iniciar(event):
    ...
    try:
        ...
        print("⚠️ Error: Ingresa un valor numérico para la probabilidad de contagio")
        return
...
def graficar_estadisticas():
    ...
    df.plot(x='día', y=['sanos', 'infectados', 'recuperados', 'muertos'], kind='line', figsize=(10, 5))
    plt.title('Evolución de la pandemia en la simulación')
    plt.xlabel('Día')
    ...
    df.to_csv("resultado_simulacion.csv", index=False)
    print("📁 Datos guardados en resultado_simulacion.csv")
plt.tight_layout()
plt.tight_layout()
plt.show()
